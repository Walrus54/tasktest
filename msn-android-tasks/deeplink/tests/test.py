...
# test.py сканирует файл-решение внутри tests/ (AndroidManifest_answer.xml)
...
def test_functionality():
    # Проверяем, что нужный handler (https://mobsec.app) реально присутствует
    result = subprocess.check_output(
        [
            "semgrep",
            "-j", "2",
            "--max-memory", "512",
            "-c", "handler-present.yml",
            "AndroidManifest_answer.xml",
            "--json"
        ],
    ...
    # Должна быть хотя бы одна находка (handler присутствует)
...
def test_security_0():
    ...
    with open("results_no_autoverify.json", "w") as f:
    ...
    with open("results_no_autoverify.json") as json_data:
        result = json.load(json_data)
        json_data.close()

    # Если есть находки → уязвимость присутствует → тест failed
    if result["results"] == []:
...
def test_security_1():
    ...
    with open("results_broad_pathprefix.json", "w") as f:
        semgrep_result = subprocess.check_output(
            [
                "semgrep",
                "-j", "2",
                "--max-memory", "512",
                "-c", "broad-pathprefix.yml",
                "AndroidManifest_answer.xml",
                "--json"
            ],
            stderr=subprocess.DEVNULL,
    ...
    with open("results_broad_pathprefix.json") as json_data:
        result = json.load(json_data)
        json_data.close()
...
def generate_report():
    ...
    flag = "MSN{D33p_L1nk_H1j4ck1ng_Pr3v3nt3d}" if functional_passed == len(functional_tests) and security_passed == len(security_tests) else ""
